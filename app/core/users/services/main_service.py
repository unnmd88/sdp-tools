...
from core.exceptions.crud import CreateError, UpdateError
...
#         user_entity: UserEntity = await self.repository.get_one_or_none_by_filters(
...
#         return user_entity
#
#     async def get_user_by_username_or_id_or_raise_user_not_found(
#         self, username_or_id: str | int
#     ) -> UserEntity:
#         user: UserEntity = await self.repository.get_user_by_id_or_username_or_none(
#             username_or_id
#         )
#         if user is None:
...
#         self, search_dto: GetUserFromRepoDTO
#     ) -> UserEntity:
#         customer: UserEntity = await self.repository.get_user_by_id_or_username_or_none(
#             search_dto.customer
#         )
#         if customer is None:
#             raise UserNotFoundError
...
#             logger.info(
#                 'Пользователю username=%r запрещен доступ к данным других пользователей.',
#                 customer.username,
#             )
#             raise UserPermissionsError
#         subject = await self.repository.get_user_by_id_or_username_or_none(
#             search_dto.subject
#         )
#         if subject is None:
...
#         customer: UserEntity = await self.repository.get_one_by_id_or_none(
...
#             await self.repository.get_one_or_none_by_filters(
#                 filters={'username': create_user_dto.username}
#             )
#         )
#         if user_already_exists:
#             logger.info(
#                 'Ошибка: пользователь %r уже существует.', user_already_exists.username
#             )
...
#             to_update_entity = await self.repository.get_user_by_username_or_none(
#                 data.subject_username
#             )
...
#         UserEntity(
#             **to_update_entity_as_dict
#         )  # Проверка, что данные для обновления валидны
#         return await self.repository.update(**data_as_dict)
#
#     async def change_password(
#         self, dto: ChangeUserPasswordDTO
#     ) -> ChangeUserPasswordDTO:
...
#             subject: UserEntity = await self._logic_change_password_myself(
#                 customer, dto
#             )
#         else:
...
#                 f'username=%r, пароль=%r',
#                 subject.username,
#                 subject.password,
#             )
#             raise SameUsernameAndPasswordError
#         logger.info(
#             'Меняю пароль для субъекта username=%r, id=%r', subject.username, subject.id
#         )
...
#         )
#
#     async def _logic_change_password_myself(
#         self, subject: UserEntity, dto
#     ) -> UserEntity:
#         logger.info(
#             'Субъект=инициатор. Смена пароля для пользователя=%r', subject.username
#         )
#         if not subject.validate_password(dto.old_password):
#             logger.info(
#                 'Неверный пароль пользователя %r: %r',
#                 subject.username,
#                 dto.new_password,
#             )
#             raise InvalidUsernameOrPasswordError
#         return subject
#
#     async def _logic_force_set_password(
#         self, customer: UserEntity, dto: ChangeUserPasswordDTO
#     ) -> UserEntity:
#         if not customer.permissions.has(Permissions.UPDATE_USERS):
#             logger.info(
#                 f'Запрещено: нет прав для изменения пароля другого пользователя.'
#             )
#             raise UserPermissionsError(
#                 'Нет прав обновления пароля другого пользователя.'
#             )
#         logger.info(f'Начинаю поиск субъекта с username=%r.', dto.subject)
#         subject: UserEntity = await self.repository.get_user_by_id_or_username_or_none(
#             dto.subject
#         )
#         if subject is None:
