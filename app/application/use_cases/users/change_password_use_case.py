...
class ChangeUserPasswordUseCaseImpl:
    ...
    async def __call__(self, user_id, change_password_dto: ChangeUserPasswordDTO) -> ChangeUserPasswordDTO:
        ...
        hashed_password = self.password_service.hash_password(
            SetPasswordVO(subject=UserEntity.__name__, password=change_password_dto.new_password).password
        )
        await self.user_service.change_password(user_id=user_id, hashed_password=hashed_password)
