...
from application.dto.users import ChangeUserPasswordDTO, ChangeUserPasswordByAdminDTO, ChangedUserPasswordByAdminDTO
from application.exceptions import AuthenticationError, ApplicationLayerError, PermissionDeniedError
...
class ResetUserPasswordByAdminUseCaseImpl:
    ...
    async def __call__(self, reset_password_dto: ChangeUserPasswordByAdminDTO) -> ChangedUserPasswordByAdminDTO:
        logger.info("Запрос на смену пароля от пользователя с id=%r", reset_password_dto.customer_id)

        customer = await self.user_service.get_active_user_by_id_or_raise(reset_password_dto.customer_id)
        ...
        subject = await self.user_service.get_user_by_username(reset_password_dto.subject_username)
        if subject is None:
            raise DomainEntityNotFoundError(
                public_message=f"Пользователь c username={reset_password_dto.subject_username} не найден."
            )

        new_password = SetPasswordVO.from_generated_password(subject=UserEntity.__name__).password
        ...
        if not self.password_service.verify_password(
            ...
            exc = ApplicationLayerError(private_message="Ошибка логики обновления пароля.")
            logger.error(exc.to_dict())
            raise exc
        logger.info("Пароль пользователя %r успешно изменён.", customer.username)
        return ChangedUserPasswordByAdminDTO(username=updated_subject.username, new_password=new_password)