...
from domain.cqrs.passport_groups_commands import UpdatePassportGroupCommand, DeletePassportGroupCommand
from domain.enums.unsorted import Roles


@dataclass(frozen=True, slots=True, kw_only=True)
class DeletePassportGroupUseCaseImpl:
    ...
    async def __call__(self, command: DeletePassportGroupCommand) -> PassportGroupDTO:
        async with self.uow:
            ...
            if user.role not in self.require_roles:
                raise PermissionDeniedError(
                    public_message="У вас нет прав для удаления группы паспортов."
                )
            passport_group = await self.passport_groups_service.delete_passport_group(command)
