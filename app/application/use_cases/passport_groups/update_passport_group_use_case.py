...
from domain.cqrs.passport_groups_commands import UpdatePassportGroupCommand
from domain.enums.unsorted import Roles


@dataclass(frozen=True, slots=True, kw_only=True)
class UpdatePassportGroupUseCaseImpl:
    ...
    async def __call__(self, command: UpdatePassportGroupCommand) -> PassportGroupDTO:
        async with self.uow:
            ...
            if user.role not in self.require_roles:
                raise PermissionDeniedError(
                    public_message="У вас нет прав для обновления группы паспортов."
                )
            passport_group = await self.passport_groups_service.update_passport_group(command)
