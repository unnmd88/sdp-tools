...
class UpdateRegionUseCaseImpl:
    ...
    require_roles: ClassVar[Container[Roles]] = frozenset([Roles.admin, Roles.superuser, Roles.director])
    ...
    async def __call__(self, command: UpdateRegionCommand) -> RegionDTO:
        async with self.uow:
            user = await self.user_service.get_active_user_by_id_or_raise(command.user_id)
            if user.role not in self.require_roles:
                raise PermissionDeniedError(public_message="У вас нет прав для изменения региона.")
