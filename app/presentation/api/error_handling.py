...
status_map = {
    DomainContractViolationError: CodeMapping(
        code=Codes.BAD_REQUEST_ERROR, http_status=status.HTTP_422_UNPROCESSABLE_ENTITY
    ),
    RepositoryCorruptedError: CodeMapping(
        code=Codes.INTERNAL_SERVER_ERROR,
        http_status=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ),
    DomainEntityNotFoundError: CodeMapping(
        code=Codes.NOT_FOUND_ERROR, http_status=status.HTTP_404_NOT_FOUND
    ),
    AuthenticationError: CodeMapping(
        code=Codes.UNAUTHORIZED, http_status=status.HTTP_401_UNAUTHORIZED
    ),
    TokenExpiredError: CodeMapping(
        code=Codes.UNAUTHORIZED, http_status=status.HTTP_401_UNAUTHORIZED
    ),
    InvalidTokenTypeError: CodeMapping(
        code=Codes.INVALID_TOKEN_TYPE_ERROR, http_status=status.HTTP_400_BAD_REQUEST
    ),
    TokenError: CodeMapping(
        code=Codes.TOKEN_ERROR, http_status=status.HTTP_400_BAD_REQUEST
    ),
    InactiveAccountError: CodeMapping(
        code=Codes.INACTIVE_ACCOUNT, http_status=status.HTTP_403_FORBIDDEN
    ),
    DomainEntityAlreadyExistsError: CodeMapping(
        code=Codes.CONFLICT_ERROR, http_status=status.HTTP_409_CONFLICT
    ),
    RepositoryIntegrityError: CodeMapping(
        code=Codes.CONFLICT_ERROR, http_status=status.HTTP_409_CONFLICT
    ),
    RepositoryConnectionError: CodeMapping(
        code=Codes.REQUEST_ERROR, http_status=status.HTTP_500_INTERNAL_SERVER_ERROR
    ),
    RepositoryUpdateError: CodeMapping(
        code=Codes.DATA_UPDATE_ERROR, http_status=status.HTTP_400_BAD_REQUEST
    ),
    PermissionDeniedError: CodeMapping(
        code=Codes.FORBIDDEN, http_status=status.HTTP_403_FORBIDDEN
    ),
}
